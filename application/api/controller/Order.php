...
class Order extends MobileBase
{
    ...
    public function appOrderList()
    {
        ...
        $order_list = M('order')->order($order_str)->where($where)->limit($start. ',' . $end)->select();

        //获取订单商品
        $model = new UsersLogic();
        $ret_list = array();
        foreach ($order_list as $k => $v) {
            $ret_list[$k]['order_id'] = $v['order_id'];

            $ret_list[$k]['order_sn'] = $v['order_sn'];

            $ret_list[$k]['total_amount'] = $v['total_amount'];
            //$ret_list[$k]['count_goods_num'] = $v['count_goods_num'];
            $ret_list[$k]['add_time'] = $v['add_time'];
            ...
            $ret_list[$k]['count_goods_num'] = $count_goods_num;
            $ret_list[$k]['order_status_code'] = $order_list[$k]['order_status_code'];
            $ret_list[$k]['goods_list'] = $order_list[$k]['goods_list'];
        ...
        return json_encode(array('status'=>0,'msg'=>"该列表为空！",'result'=>null));
    }
    ...
    public function addComment()
    {
        ...
        if ($post) {
            ...
            $comment_img_file = $post['comment_img_file'];
            unset($post['comment_img_file']);
            $comment_img = uploadImage($comment_img_file,'comment');
            ...
            $hide_username = I('hide_username') ? 1:0;
            if ($hide_username == 0) {
    ...
    public function appComment()
    {
        ...
        $data = $logic->getAppComment($user_id, $status,$start,$end); //获取评论列表
        ...
        $ret_list = array();
        foreach($data as $k=>$v){
            $ret_list[$k]['rec_id'] = $v['rec_id'];
            $ret_list[$k]['goods_name'] = $v['goods_name'];
            $ret_list[$k]['goods_num'] = $v['goods_num'];
            $ret_list[$k]['member_goods_price'] = $v['member_goods_price'];
            $ret_list[$k]['spec_key_name'] = $v['spec_key_name'];
            $ret_list[$k]['pay_time'] = $v['pay_time'];
            $ret_list[$k]['content'] = $v['content'];
            $ret_list[$k]['add_time'] = $v['add_time'];
            $ret_list[$k]['img'] = $v['img'];
            $ret_list[$k]['deliver_rank'] = $v['deliver_rank'];
            $ret_list[$k]['goods_rank'] = $v['goods_rank'];
            $ret_list[$k]['service_rank'] = $v['service_rank'];
            $ret_list[$k]['comment_id'] = $v['comment_id'];
            $ret_list[$k]['original_img'] = Db::name('goods')->where(['goods_id'=>$v['goods_id']])->value('original_img');
            $admin_reply = M('comment')->where(array('parent_id'=>$v['comment_id']))->field($field)->select(); // 评论回复列表
            $ret_list[$k]['admin_reply'] = $admin_reply;
        }
        if(!empty($ret_list)){
            return json_encode(array('status'=>1,'msg'=>"获取成功！",'result'=>$ret_list));
        }
        return json_encode(array('status'=>0,'msg'=>"你还没有评论！"));
    }
    ...
    public function commentInfo(){
        ...
        if(empty($res)){
            return json_encode(array('status'=>0,'msg'=>"没有评论！",'result'=>null));
            //$this->error('参数错误！！');
        }
        return json_encode(array('status'=>1,'msg'=>"获取评论成功！",'result'=>$res));
    ...
    public function orderConfirm()
    {
       ...
       /* if(request()->isAjax()){
        ...
        if ($data['status'] != 1) {
            return json_encode(array('status'=>0,'msg'=>$data['msg']));
            //$this->error($data['msg'],U('Mobile/Order/order_list'));
        } else {
            return json_encode(array('status'=>1,'msg'=>"确认成功！"));
            /*$model = new UsersLogic();
    ...
    public function waitcomment()
    {
        ...
        $logic = new CommentLogic;
        $data = $logic->getWaitComment($user_id, $status,$page); //获取评论列表
        $ret_list = array();
        foreach($data as $k=>$v){
            $ret_list[$k]['rec_id'] = $v['rec_id'];
            $ret_list[$k]['goods_name'] = $v['goods_name'];
        ...
        if(!empty($ret_list)){
            return json_encode(['status'=>1,'msg'=>"获取成功！",'result'=>$ret_list]);
        }
        return json_encode(['status'=>0,'msg'=>"你没有待评价！"]);
