...
class Goods extends MobileBase {
    ...
    public function categoryList(){
        $goods_category_tree = app_get_goods_category_tree();
        if(!empty($goods_category_tree)){
            return json_encode(array('status'=>1,'msg'=>"获取成功！",'result'=>$goods_category_tree));
        }
        return json_encode(array('status'=>0,'msg'=>"分类列表为空！",'result'=>null));
    }
    ...
    public function appGoodsList(){
        ...
        //获取商品列表
        $field = "goods_id,cat_id,goods_name,shop_price,market_price,original_img,sales_sum";
        $con = [
            'cat_id'      => $cat_id,
            'is_on_sale'  => 1,
            'is_del'      => 1
        ];
        $goods_list = Db::name('goods')->where($con)->limit($start,$end)->field($field)->order('sort desc')->select();
        if(!empty($goods_list)){
            return json_encode(array('status'=>1,'msg'=>"获取商品列表成功！",'result'=>$goods_list));
        }
        return json_encode(array('status'=>0,'msg'=>"该分类下没有商品！",'result'=>null));
    }
    ...
    public function appGoodsInfo(){
        ...
        if($spec_goods_price){
            ...
            foreach ($spec_goods_price as $k => $v) {
                $spec_goods_prices[$i] = $v;
                $i++;
            }
        }
        $filter_spec2 = array();
        if(is_array($filter_spec)){
        ...
        $goods_comment = $goodsLogic->appGetContent($goods_id);// 获取某个商品的评论统计
        if(!empty($goods_comment)){
            $goods_comment['head_pic'] = Db::name('users')->where(['user_id'=>$goods_comment['user_id']])->value('head_pic');
        }
        //$goods_comment['head_pic'] = Db::name('users')->where(['user_id'=>$goods_comment['user_id']])->value('head_pic');
        $comment_num = $goodsLogic->appGetComNum($goods_id);
        ...
        $goods_info = array(
            ...
            'filter_spec' => $filter_spec2,
            'spec_goods_price'=>$spec_goods_prices*/
        );
        $goods_content = htmlspecialchars_decode($goods['goods_content']);
        ...
        $goods_info['sale_num'] = $goods['sale_num'];
        $goods_info['collect'] = $collect;
        $goods_info['comment_num'] = $comment_num;
        $goods_info['goods_comment'] = $goods_comment;
        $goods_info['goods_images_list'] = $goods_images_list;
        $goods_info['filter_spec'] = $filter_spec2;
        $goods_info['spec_goods_price'] = $spec_goods_prices;
        return json_encode(array('status'=>1,'msg'=>"获取成功！",'result'=>$goods_info));
    }

    /**
    *获取商品评论列表
     */
    public function getGoodsComList(){
        $user_id = $this->user_id;
        $goodsLogic = new GoodsLogic();
        $goods_id = input('post.goods_id');
        ...
        $field = "comment_id,goods_id,username,content,img,service_rank,goods_rank,deliver_rank,add_time,user_id,order_id";
        $goods_comment = Db::name('comment')->where($commentWhere)->field($field)->order('comment_id desc')->limit($start,$end)->select();

        foreach ($goods_comment as $k => $v) {
            $goods_comment[$k]['head_pic'] =  Db::name('users')->where(['user_id'=>$v['user_id']])->value('head_pic');
            $admin_reply = M('comment')->where(array('parent_id'=>$v['comment_id']))->field($field)->select(); // 评论回复列表
            $goods_comment[$k]['admin_reply'] = $admin_reply;
        }
        if(empty($goods_comment)){
            return json_encode(array('status'=>0,'msg'=>"该商品还没有评论！"));
        }
        return json_encode(array('status'=>1,'msg'=>"获取成功！",'result'=>$goods_comment));
    ...
    public function delCollect(){
        ...
        $ret = Db::name('goods_collect')->where($col_con)->delete();
        if($ret){
    ...
    public function appSearch(){
        ...
        if($count > 0)
        {
            $goods_list = M('goods')->where("goods_id", "in", implode(',', $filter_goods_id))->order([$sort=>$sort_asc])->limit($start.','.$end)->select();
            $filter_goods_id2 = get_arr_column($goods_list, 'goods_id');
            if($filter_goods_id2)
                $goods_images = M('goods_images')->where("goods_id", "in", implode(',', $filter_goods_id2))->cache(true)->select();
        }
       ...
       /* $this->assign('goods_list',$goods_list);
        ...
        foreach($goods_list as $k=>$v){
            $ret_list[$k]['goods_id'] = $v['goods_id'];
            $ret_list[$k]['original_img'] = $v['original_img'];
            $ret_list[$k]['goods_name'] = $v['goods_name'];
            $ret_list[$k]['shop_price'] = $v['shop_price'];
        }
        if(!empty($ret_list)){
            return json_encode(array('status'=>1,'msg'=>"获取成功！",'result'=>$ret_list));
        }
        return json_encode(array('status'=>0,'msg'=>"请换一个搜索词条！",'result'=>null));
    }
