...
class CartLogic extends Model
{
    ...
    {
        if (empty($this->goods)) {
            throw new TpshopException('立即购买', 0, ['status' => 0, 'msg' => '购买商品不存在', 'result' => null]);
        }
        if (empty($this->goodsBuyNum)) {
            throw new TpshopException('立即购买', 0, ['status' => 0, 'msg' => '购买商品数量不能为0', 'result' => null]);
        }
        $buyGoods = [
     ...
     * @return false|\PDOStatement|string|\think\Collection
     */
    public function getSelectedCartList($cart_ids = array())
    {
        $Cart = new Cart();
        // 如果用户已经登录则按照用户id查询
        if ($this->user_id) {
        ...
        $cartWhere['combination_group_id'] = 0;
        $cartList = $Cart->where($cartWhere)->where('id', 'IN', $cart_ids)->select();  // 获取购物车商品
        ...
        foreach ($cartCheckAfterList as $cartKey =>$cart){
            ...
            $cartCheckAfterList[$cartKey]['limit_num'] = M('spec_goods_price')->where($where)->value('store_count');
            foreach ($cart['combination_cart'] as $k=>$v ){
                $cartCheckAfterList[$cartKey]['count_price'] = $cart['goods_price'] - $cart['member_goods_price'];
                if($v['prom_type']==7){
                    $cartCheckAfterList[$cartKey]['count_price'] += $v['goods_price'] - $v['member_goods_price'];
    ...
    public function getAppCartList($selected = 0)
    {
        $cart = new Cart();
        // 如果用户已经登录则按照用户id查询
        if ($this->user_id) {
        ...
        $cartWhere['combination_group_id'] = 0;
        $cartList = $cart->where($cartWhere)->select();  // 获取购物车商品
        $cartCheckAfterList = $this->checkAppCartList($cartList);
     ...
     * @param $cartList
     */
    public function checkAppCartList($cartList)
    {
        $returncart = array();
        $goodsPromFactory = new GoodsPromFactory();
        foreach ($cartList as $cartKey => $cart) {
            ...
            $value['goods_sn']      = $cart['goods_sn'];
            $value['goods_name']    = $cart['goods_name'];
            $value['goods_price']   = $cart['goods_price'];
            $value['goods_num']     = $cart['goods_num'];
            $value['item_id']       = $cart['item_id'];
            $value['spec_key']      = $cart['spec_key'];
            $value['spec_key_name'] = $cart['spec_key_name'];
            $value['add_time']      = $cart['add_time'];
            $value['original_img']  = $cart['goods']['original_img'];
            $returncart[$cartKey] = $value;
        }
...
//                $arr= $Cart->where(['combination_group_id' => $cartVal['combination_group_id'], 'id' => ['neq', $cartVal['id']]])->select();
     ...
     * app端口获取组合后的数据
     */
    public function getRetCartList($cartList = null){
        $ret_info = array();
        $i = 0;
        if ($cartList) {
            foreach ($cartList as $cartKey => $cartItem) {
                if($cartItem['id']){
                    $ret_info[$i]['id'] = $cartItem['id'];
                }
                $ret_info[$i]['goods_id'] = $cartItem['goods_id'];
                $ret_info[$i]['goods_sn'] = $cartItem['goods_sn'];
                $ret_info[$i]['goods_name'] = $cartItem['goods_name'];
                $ret_info[$i]['goods_price'] = $cartItem['goods_price'];
                $ret_info[$i]['member_goods_price'] = $cartItem['member_goods_price'];
                $ret_info[$i]['goods_num'] = $cartItem['goods_num'];
                //$ret_info[$i]['weight'] = $cartItem['weight'];
                $ret_info[$i]['spec_key'] = $cartItem['spec_key'];
                $ret_info[$i]['spec_key_name'] = $cartItem['spec_key_name'];
                $ret_info[$i]['original_img'] = $cartItem['goods']['original_img'];
                $i++;
            }
        }
        return $ret_info;
...
//            $master_cart->save(['combination_group_id' => $master_cart['id']]);
    ...
    public function dispatching($goods_id, $region_id)
    {
        ...
        if(!$isShipping){
            throw new TpshopException('该地区不支持配送', 0, ['status'=>0,'msg'=>$goods['goods_name'].'该地区不支持配送','result'=>null]);
        }
    }
}