...
class GoodsLogic extends Model
{
     ...
     * APP中获取用户对商品的评论
     */
    public function appGetContent($goods_id){
        $commentWhere = ['is_show'=>1,'goods_id'=>$goods_id,'parent_id'=>0,'user_id'=>['gt',0]];
        $field = "comment_id,goods_id,username,content,add_time,user_id,order_id";
        $goods_comment = Db::name('comment')->where($commentWhere)->field($field)->order('comment_id desc')->find();
        return $goods_comment;
    }
    /**
     *在app接口中获取对该商品的评论总数
     */
    public function appGetComNum($goods_id){
        $commentWhere = ['is_show'=>1,'goods_id'=>$goods_id,'parent_id'=>0,'user_id'=>['gt',0]];
        $comment_num = Db::name('comment')->where($commentWhere)->count();
        return $comment_num;
    }
    ...
    public function collect_goods($user_id, $goods_id)
    {
        if(!is_numeric($user_id) || $user_id <= 0){
            return array('status'=>-1,'msg'=>'必须登录后才能收藏','result'=>null);
        }
        $count = Db::name('goods_collect')->where("user_id", $user_id)->where("goods_id", $goods_id)->count();
        if($count > 0){
            return array('status'=>-3,'msg'=>'商品已收藏','result'=>null);
        }
        ...
        return array('status'=>1,'msg'=>'收藏成功!请到个人中心查看','result'=>null);
    }
    /**
     * 获取商品规格
     * @param $goods_id|商品id
