...
class OrderLogic
{
	...
	public function cancel_order($user_id,$order_id,$action_note='您取消了订单'){
		...
		if(empty($order))
			return array('status'=>-1,'msg'=>'订单不存在','result'=>null);
		if($order['order_status'] == 3){
			return array('status'=>-1,'msg'=>'该订单已取消','result'=>null);
		}
		//检查是否未支付的订单
		if($order['pay_status'] > 0 || $order['order_status'] > 0)
			return array('status'=>-1,'msg'=>'支付状态或订单状态不允许','result'=>null);
		//获取记录表信息
		...
		if(!$row)
			return array('status'=>-1,'msg'=>'操作失败','result'=>null);
		return array('status'=>1,'msg'=>'操作成功','result'=>null);
	...
	public function addPreSellOrder($user_id,$address_id,$invoice_title,$act_id,$pre_sell_price, $taxpayer="")
	{
		...
		if($order_count >= 50){
			return array('status'=>-9,'msg'=>'一天只能下50个订单','result'=>null);
		}
