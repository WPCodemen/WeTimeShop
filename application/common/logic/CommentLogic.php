...
class CommentLogic extends Model
{
	...
	public function getCommentInfo($comment_id)
	{
        ...
        $admin_reply = M('comment')->where(array('parent_id'=>$comment_id))->select(); // 评论回复列表
		return array('comment_info'=>$comment_info,'admin_reply'=>$admin_reply,'reply'=>$reply);
	}
     ...
     * 获取评论列表
     * @param $user_id1 用户id
     * @param int $status1 状态 0 未评论 1 已评论 ,其他 全部
     * @return mixed1
     */
    public function getWaitComment($user_id, $status = 0,$page=1)
    {
        $start = ($page-1) * 10;
        $end = $page * 10;
        //$comment_count = $this->getCommentNum($user_id, $status);
        //$page = new \think\Page($comment_count,10);
        $comment_list = $this->getCommentList($user_id, $status, $start, $end);
     ...
     * @return mixed
     */
    public function getAppComment($user_id, $status = 2,$start=0,$end=10)
    {
        /*$comment_count = $this->getCommentNum($user_id, $status);
        $page = new \think\Page($comment_count,10);*/
        $comment_list = $this->getCommentList($user_id, $status, $start, $end);
    ...
    public function getCommentQuery($queryType, $user_id, $status = 2, $firstRow = 1, $listRows = 10)
    {
        ...
        if ($queryType) {
            return $query->field('og.*,og.is_comment as goods_comment,o.*,c.comment_id,c.content,c.add_time,c.img,c.deliver_rank,c.goods_rank,c.service_rank')
