...
class UsersLogic extends Model
{
    ...
    public function app_login($username, $password, $capache='', $push_id=0)
    {
        ...
        if(!$user){
           ...
           $result = array('status'=>-3,'msg'=>'账号异常已被锁定！！！');
            ...
            $token = md5(uniqid().mt_rand(1,999999999).$user_id);
            //把用户的token放在redis缓存中
            $redis = new Redis();
            $redis->set($user_id,$token,604800); //设置有效期为604800(7天)秒的键值
            /*$push_id && $data['push_id'] = $push_id;
            M('users')->where("user_id", $user['user_id'])->save($data);*/
            $result = array('status'=>1,'msg'=>'登陆成功','result'=>array('token'=>$token,'user_id'=>$user_id));
    ...
    public function oauth_bind_new($user = array())
    {
        ...
        if(empty($ruser)){
            return array('status'=>-1,'msg'=>'账号不存在','result'=>null);
        }
        
        if($ruser['password'] != $user['password']){
            return array('status'=>-1,'msg'=>'账号或密码错误','result'=>null);
        }
        ...
        if((empty($unionid) || empty($openid)) && empty($oauth)){
            return array('status'=>-1,'msg'=>'第三方平台参数有误[openid:'.$openid.' , unionid:'.$unionid.', oauth:'.$oauth.']','result'=>null);
        }
        ...
        if($unionid){ 
            ...
            if($thirdUser && $ruser['user_id'] != $thirdUser['user_id'] ){ 
                return array('status'=>-1,'msg'=>'此'.$oauthCN.'已绑定其它账号','result'=>null);
            } 
            ...
            $thirdUser = M('OauthUsers')->where(['user_id'=>$ruser['user_id'], 'oauth'=> $oauth])->find();
            if($thirdUser){
                return array('status'=>-1,'msg'=>'此账号已绑定其它'.$oauthCN.'账号','result'=>null);
            } 
        }
    ...
    public function thirdLogin_new($data = array())
    {
        if((empty($data['openid']) && empty($data['unionid'])) || empty($data['oauth'])){
            return ['status' => -1, 'msg' => '参数错误, openid,unionid或oauth为空','result'=>null];
        }
    ...
    public function reg($username,$password,$password2,$push_id = 0,$invite=array(),$nickname="",$head_pic=""){
        ...
        if($is_validated != 1)
            return array('status'=>-1,'msg'=>'请用手机号或邮箱注册','result'=>null);
        $map['nickname'] = $nickname ? $nickname : $username;
        
        if(!empty($head_pic)){
            $map['head_pic'] = $head_pic;
        }else{
            $map['head_pic']='/Public/upload/users/userportrait.jpg';
        }
        ...
        if(!$UserRegValidate->scene('reg')->check($data)){
            return array('status'=>-1,'msg'=>$UserRegValidate->getError(),'result'=>null);
        }
       ...
       /* $distribut_condition = tpCache('distribut.condition');
        ...
        return array('status'=>1,'msg'=>'注册成功','result'=>null);
    }
    ...
    public function add_comment($add){
        if(!$add['order_id'] || !$add['goods_id']) 
            return array('status'=>-1,'msg'=>'非法操作','result'=>null);
        
        //检查订单是否已完成
        $order = M('order')->field('order_status')->where("order_id", $add['order_id'])->where('user_id', $add['user_id'])->find();
        if($order['order_status'] != 2)
            return array('status'=>-1,'msg'=>'该笔订单还未确认收货','result'=>null);

        //检查是否已评论过
        $goods = M('comment')->where(['rec_id'=>$add['rec_id']])->find();
        if($goods)return array('status'=>-1,'msg'=>'您已经评论过该商品','result'=>null);
        if($add['goods_rank']<1 || $add['service_rank']<1){
            return array('status'=>-1,'msg'=>'请给商品服务评分','result'=>null);
        }
        ...
        if($row)
        {
            ...
            return array('status'=>1,'msg'=>'评论成功','result'=>null);
        }
        return array('status'=>-1,'msg'=>'评论失败','result'=>null);
    }
    ...
    public function addAddress($user_id,$address_id=0,$data){
        ...
        if($address_id == 0)
        {
            ...
            if($c >= 20)
                return array('status'=>0,'msg'=>'最多只能添加20个收货地址','result'=>null);
        }

        //检查手机格式
        if($post['consignee'] == '')
            return array('status'=>0,'msg'=>'收货人不能为空','result'=>null);
        if(!$post['province'] || !$post['city'] || !$post['district'])
            return array('status'=>0,'msg'=>'所在地区不能为空','result'=>null);
        if(!$post['address'])
            return array('status'=>0,'msg'=>'地址不能为空','result'=>null);
        if(!check_mobile($post['mobile']))
            return array('status'=>0,'msg'=>'手机号码格式有误','result'=>null);
        ...
        if($address_id > 0){
            ...
            if($row !== false){
                return array('status'=>1,'msg'=>'编辑成功','result'=>null);
            }else{
                return array('status'=>0,'msg'=>'操作完成','result'=>null);
            }
        ...
        if(!$address_id)
            return array('status'=>0,'msg'=>'添加失败','result'=>null);


        return array('status'=>1,'msg'=>'添加成功','result'=>null);
    }
    ...
    public function add_address($user_id,$address_id=0,$data){
        ...
        if($address_id == 0)
        {
            $c = M('UserAddress')->where("user_id", $user_id)->count();
            if($c >= 20)
                return array('status'=>-1,'msg'=>'最多只能添加20个收货地址','result'=>null);
        }

        //检查手机格式
        if($post['consignee'] == '')
            return array('status'=>-1,'msg'=>'收货人不能为空','result'=>null);
        if (!($post['province'] > 0)|| !($post['city']>0) || !($post['district']>0))
            return array('status'=>-1,'msg'=>'所在地区不能为空','result'=>null);
        if(!$post['address'])
            return array('status'=>-1,'msg'=>'地址不能为空','result'=>null);
        if(!check_mobile($post['mobile']) && !check_telephone($post['mobile']))
            return array('status'=>-1,'msg'=>'手机号码格式有误','result'=>null);
        ...
        if($post['is_default'] == 1)
            M('user_address')->where($map)->save(array('is_default'=>0));
        if(!$address_id)
            return array('status'=>-1,'msg'=>'添加失败','result'=>null);
     ...
     * @return array
     */
    public function password($user_id,$mobile,$old_password,$new_password,$confirm_password,$is_update=true){
        $user = M('users')->where('user_id', $user_id)->find();
        if($user['mobile'] != $mobile){
            return array('status'=>-1,'msg'=>"手机号错误！",'result'=>null);
        }
        $data=[
        ...
        if($is_update && ($user['password'] != '' && encrypt($old_password) != $user['password']))
            return array('status'=>-1,'msg'=>'原密码验证失败','result'=>null);
        $row = M('users')->where("user_id", $user_id)->save(array('password'=>encrypt($new_password)));
        if(!$row)
            return array('status'=>-1,'msg'=>'修改失败','result'=>null);
        return array('status'=>1,'msg'=>'修改成功','result'=>null);
    }
    ...
    public function passwordForApp($user_id,$old_password,$new_password,$is_update=true){
        ...
        if($is_update && ($user['password'] != '' && $old_password != $user['password'])){
            return array('status'=>-1,'msg'=>'旧密码错误','result'=>null);
        }

        $row = M('users')->where("user_id='{$user_id}'")->update(array('password'=>$new_password));
        if(!$row){
            return array('status'=>-1,'msg'=>'密码修改失败','result'=>null);
        }
        return array('status'=>1,'msg'=>'密码修改成功','result'=>null);
    }
