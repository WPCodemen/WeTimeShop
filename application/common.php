...
/**
 * 存储base64图片内容，返回图片路径
 * @param string $base64Content 不包含头部
 * @param  string upload下的文件夹已存在的  默认 $file='app'
 * @return strings
 * @return getdate(time())=Array([seconds] => 45[minutes] => 52[hours] => 14[mday] => 24[wday] => 2[mon] => 1[year] => 2006[yday] => 23[weekday] => Tuesday[month] => January[0] => 1138110765);
 */
function uploadImage($base64_img,$type)
{
    ...
    if(!empty($base64Content))
    {
        ...
        ////存放在当前目录的upload文件夹下
        if($type == 'idcard'){
            $up_dir = './public/upload/idcard/'.$date.'/';
        }
        if($type == 'pay_pic'){
            $up_dir = './public/upload/pay_pic/'.$date.'/';
        }
        if($type == 'users'){
            $up_dir = './public/upload/users/'.$date.'/';
        }
        if($type == 'comment'){
            $up_dir = './public/upload/comment/'.$date.'/';
        }
        ...
        $path = $up_dir;
        //$path = realpath(dirname(__FILE__)). "/../../../data/upload/{$file}/";
        //每天创建一个目录
 ...
 * @return type
 */
function app_get_goods_category_tree(){
    $tree = $arr = $result = array();
    $field = "id,mobile_name";
    $cat_list = M('goods_category')->cache('goods_category',86400)->where(['is_show' => 1,'is_del'=>1])->field($field)->order('sort_order')->select();//所有分类
    /*if($cat_list){
