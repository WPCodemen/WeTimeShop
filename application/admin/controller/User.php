...
class User extends Base {
    ...
    public function detail(){
        ...
        if(IS_POST){
            ...
            if(!empty($_POST['alipay_acc']))
            {   $email = trim($_POST['alipay_acc']);
                $c = M('users')->where("user_id != $uid and alipay_acc = 'alipay_acc'")->count();
                $c && exit($this->error('支付宝账号不得和已有用户重复'));
            }

            if(!empty($_POST['wx_acc']))
            {   $email = trim($_POST['wx_acc']);
                $c = M('users')->where("user_id != $uid and wx_acc = 'wx_acc'")->count();
                $c && exit($this->error('微信账号不得和已有用户重复'));
            }
    ...
    *积分体现列表
    ...
    public function withdrawList(){
        $where = "1=1";
        $count = M('withdraw_points')->where($where)->count();
        $page = new Page($count);
        $lists  = M('withdraw_points')->where($where)->order('wp_id desc')->limit($page->firstRow.','.$page->listRows)->select();
    ...
    public function editWithdraw(){
        $wp_id = input('get.wp_id');
        $post = input('post.');
        if($post){
            $ret = Db::name('withdraw_points')->where(['wp_id'=>$post['wp_id']])->save(['status'=>$post['status'],'check_time'=>time()]);
            if($ret){
                if($post['status'] == 2){
                    $withdraw_points = Db::name('withdraw_points')->where(['wp_id'=>$post['wp_id']])->find();
                    $where = array('user_id'=>$withdraw_points['user_id']);
                    Db::name('withdraw_points')->where($where)->setDec('pay_points',$withdraw_points['points']);
                }
                $this->success('审核成功',U('User/withdrawList'));
            }
            $this->error('添加失败,',U('User/withdrawList'));
        }
        $recorder = Db::name('withdraw_points')->where(['wp_id'=>$wp_id])->find();
        $this->assign('recorder',$recorder);
        return $this->fetch();
    }
    ...
    public function withdrawDelete(){
        $wp_id = input("get.wp_id");
        $row = M('withdraw_points')->where(array('wp_id'=>$wp_id))->delete();
        if($row){
            $this->success('成功删除积分体现记录！');
        }else{
            $this->error('操作失败');
        }
