...
class Article extends Base {
    ...
    */
    public  function leaveWord(){
        ...
        $wordList =  M('leave_word');
        $res = $list = array();
        $p = empty($_REQUEST['p']) ? 1 : $_REQUEST['p'];
        $size = empty($_REQUEST['size']) ? 20 : $_REQUEST['size'];

        $list = $wordList->order('id desc')->page("$p,$size")->select();
        $count = $wordList->count();// 查询满足要求的总记录数
    ...
    *查看留言
    ...
    public function wordDetail(){
        $id = I('get.id');
        $leave_word = M('leave_word')->where('id',$id)->find();
        $this->assign('leave_word',$leave_word);
        return $this->fetch();
    }
