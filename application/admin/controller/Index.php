...
//    	$today = strtotime("-1 day");
    	...
    	$count['goods'] =  M('goods')->where("1=1 and is_del=1")->count();//商品总数
    ...
    public function changeTableVal(){  
            ...
            $value  = I('value'); // 修改字段值
            if($table == 'goods_category' && $field == "is_hot" && $value == 1){
                $num = Db::name('goods_category')->where(['is_hot'=>1])->count('id');
                if($num >= 4){
                    return json_encode(['status'=>0,'msg'=>"分类的首页推荐不能超过4个"]);
                }
            }
