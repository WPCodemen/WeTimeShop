...
class System extends Base
{
	...
	public function handle()
	{
        ...
        if($inc_type == 'basic'){
            if(!is_numeric($param['exchange_points'])){
                $this->error("积分兑换的人民币必须是数字！",U('System/index',array('inc_type'=>$inc_type)));
            }
            $count = getLenth($param['exchange_points']);
            if($count > 2){
                $this->error("小数只能两位！",U('System/index',array('inc_type'=>$inc_type)));
            }
        }
     ...
     */
      public function ClearGoodsHtml(){
            ...
            if(unlink("./Application/Runtime/Html/Home_Goods_goodsInfo_{$goods_id}.html"))
            {
                ...
                $json_arr = array('status'=>1,'msg'=>'清除成功','result'=>null);
            }
            else 
            {
                $json_arr = array('status'=>-1,'msg'=>'未能清除缓存','result'=>null );
            }                                                    
     ...
     */
      public function ClearAritcleHtml(){
            ...
            $json_arr = array('status'=>1,'msg'=>'操作完成','result'=>null );
            $json_str = json_encode($json_arr);            
            exit($json_str);            
      }
