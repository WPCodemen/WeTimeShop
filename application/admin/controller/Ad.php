...
class Ad extends Base{
    public function ad(){
        ...
        $position = D('ad_position')->where(['is_del'=>1])->select();
    ...
    public function adList(){
        ...
        $ad_position_list = M('AdPosition')->where(['is_del'=>1])->getField("position_id,position_name,is_open");
    ...
    {
        $count = Db::name('ad_position')->where(['is_del'=>1])->count();// 查询满足要求的总记录数
        $Page = $pager = new Page($count, 10);// 实例化分页类 传入总记录数和每页显示的记录数
        $list = Db::name('ad_position')->where(['is_del'=>1])->order('position_id DESC')->limit($Page->firstRow . ',' . $Page->listRows)->select();
        $show = $Page->show();
