...
class GoodsLogic extends Model
{
    ...
    public function goods_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
                ...
                $sql = "SELECT * FROM  __PREFIX__goods_category where `is_del`=1 ORDER BY parent_id , sort_order ASC";
    ...
    function getSortCategory()
    {
        ...
        $categoryList =  M("GoodsCategory")->where(['is_del'=>1])->cache(true)->getField('id,mobile_name as name,parent_id,level');
        $nameList = array();
    ...
    public function afterSave($goods_id)
    {
        ...
        // 商品货号
        $goods_sn = "WE".str_pad($goods_id,7,"0",STR_PAD_LEFT);
